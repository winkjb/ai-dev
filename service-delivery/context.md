...
- `01-coordinator/`, `02-client-communication/`, `03-intake-triage/`, `04-planner/`, `05-analyst/`, `06-risk-compliance/`, `07-resource-allocation/`, `08-orchestrator/` — one folder per agent role (numbered per the architecture doc), each self-contained: scripts, notes, and any role-specific data for that agent live here. `01-coordinator` and `04-planner` are named to match `project-management`'s equivalent roles for cross-project consistency (same functional pattern, separate domain-specific implementation, not shared code).
