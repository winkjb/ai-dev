...
OUTPUT_DETAIL = OUTPUT_DIR / "coordinator-project-resource-detail.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "coordinator-project-resource-summary.md"
OUTPUT_SUMMARY_CSV = OUTPUT_DIR / "coordinator-project-resource-summary.csv"
...
def fill_no_tech_lead_label(df):
    df = df.copy()
...
def write_outputs(df, pivot, phase_cols, excluded_count):
    ...
    detail_cols = [
        ...
        "Status", "Phase", "Project Lead", "Last Activity Time",
    ]
    ...
    lines = []
    lines.append(f"# Project Management Coordinator Report (By Technical Resource) - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Projects By Technical Resource")
    lines.append("")
    header = "| Project Team Tech Lead | " + " | ".join(phase_cols) + " | Total |"
    lines.append(header)
