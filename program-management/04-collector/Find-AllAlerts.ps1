...
$OutputRoot = ".\AuvikFirewallConfigs"
...
$Tenants = Get-AllAuvikResults -InitialUri $Uri
...
foreach ($tenant in $tenants) {
    ...
    $alertsUri = "$BaseUri/alert/history/info?tenants=$tenantId&filter[dismissed]=false&filter[status]=created&page[first]=$PageSize"
    $alerts = Get-AllPages -InitialUri $alertsUri
