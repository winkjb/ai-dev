...
.SYNOPSIS
    ...
    scripts (e.g. ../02-analyst/Compare-GroupMembers-CA.ps1) read this file rather than calling
    Graph directly.

.DESCRIPTION
    -GroupNames defaults to the 4 CA exclusion group display names from the original
    katz/Invoke-GroupMemberAudit-CA-Default.ps1 script - override per-call if a customer's CA
...
.EXAMPLE
    .\Collect-EntraGroupMembers-CA.ps1 -Directory katz
#>
...
if (-not $OutputPath)   { $OutputPath   = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraGroupMembers-CA.csv" }
