...
.SYNOPSIS
    ...
    data/raw/<Directory>/. Analyst scripts (../02-analyst/Compare-Policies-CA.ps1)
...
.DESCRIPTION
    ...
    Writes its own EntraUsers-CA.csv/EntraGroups-CA.csv rather than reusing
...
.EXAMPLE
    .\Collect-EntraPolicies-CA.ps1 -Directory katz
#>
...
Export-Utf8NoBomCsv -Path (Join-Path $OutputDir "EntraUsers-CA.csv") -InputObject @($UserRows)
...
Export-Utf8NoBomCsv -Path (Join-Path $OutputDir "EntraGroups-CA.csv") -InputObject @($GroupRows)
...
Export-Utf8NoBomCsv -Path (Join-Path $OutputDir "EntraRoles-CA.csv") -InputObject @($RoleRows)
...
Export-Utf8NoBomCsv -Path (Join-Path $OutputDir "EntraPolicies-CA.csv") -InputObject @($PolicyRows)
