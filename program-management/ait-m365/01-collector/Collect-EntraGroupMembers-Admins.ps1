...
.SYNOPSIS
    ...
    ../02-analyst/Compare-GroupMembers-Admins.ps1) read these files rather than calling Graph
    directly.
...
.EXAMPLE
    .\Collect-EntraGroupMembers-Admins.ps1 -Directory katz
#>
...
Export-Utf8NoBomCsv -Path (Join-Path $OutputDir "EntraLicenses-Admins.csv") -InputObject @($LicenseRows)
...
Export-Utf8NoBomCsv -Path (Join-Path $OutputDir "EntraRoles-Admins.csv") -InputObject @($AssignmentRows)
...
Export-Utf8NoBomCsv -Path (Join-Path $OutputDir "EntraUsers-Admins.csv") -InputObject @($UserRows)
