...
.EXAMPLE
    .\Invoke-LicenseAuditSummary-Default.ps1
#>
...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\license-summary-audit.csv"
...
try {
    ...
    Write-ToLog -LogFile $OutputFile -Message "=== Starting $($SpFolder) license summary audit run ==="
    ...
    & (Join-Path $PSScriptRoot "..\02-analyst\Compare-LicenseSummaryAudit.ps1") -Directory $CustomerDir
    Write-ToLog -LogFile $OutputFile -Message "Generated the license summary audit"

    # ---------------------------------------------------------------------------
    # Send email
    # ---------------------------------------------------------------------------

    & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra License Audit - Summary" `
    ...
    try {
        & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra License Audit - Summary - FAILED" `
            -From $FromAddress `
            -Body "The scheduled license summary audit run failed: $($_.Exception.Message)`n`nSee $OutputFile on the host machine for details."
    }
    catch {
