...
.EXAMPLE
    .\Invoke-SiteAuditOneDrive-Activity-Default.ps1
#>
...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\OneDrive-Activity.csv"
...
try {
    ...
    Write-ToLog -LogFile $OutputFile -Message "=== Starting $($SpFolder) OneDrive activity audit run ==="

    & (Join-Path $PSScriptRoot "..\01-collector\Collect-EntraOneDriveActivity.ps1") -Directory $CustomerDir -SettingsPath $SettingsPath
    Write-ToLog -LogFile $OutputFile -Message "Collected OneDrive activity report"
    ...
    & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - OneDrive Activity Audit" `
    ...
    try {
        & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - OneDrive Activity Audit - FAILED" `
            -From $FromAddress `
            -Body "The scheduled OneDrive activity audit run failed: $($_.Exception.Message)`n`nSee $OutputFile on the host machine for details."
    }
    catch {
