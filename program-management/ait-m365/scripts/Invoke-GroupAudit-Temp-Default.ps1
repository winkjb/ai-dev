...
.EXAMPLE
    .\Invoke-GroupAudit-Temp-Default.ps1
#>
...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\GroupAudit-Temp.csv"
...
try {
    ...
    Write-ToLog -LogFile $OutputFile -Message "=== Starting $($SpFolder) temp-groups audit run ==="
    ...
    & (Join-Path $PSScriptRoot "..\02-analyst\Compare-Groups-Temp.ps1") -Directory $CustomerDir
    ...
    & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra Temp Groups Audit" `
    ...
    try {
        & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra Temp Groups Audit - FAILED" `
            -From $FromAddress `
