...
.EXAMPLE
    .\Invoke-UserAudit-LicensedDisabled-Default.ps1
#>
...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\UserAudit-LicensedAndDisabled.csv"
...
try {
    ...
    Write-ToLog -LogFile $OutputFile -Message "=== Starting $($SpFolder) licensed+disabled user audit run ==="
    ...
    & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra User Audit - Licensed and Disabled" `
    ...
    try {
        & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra User Audit - Licensed and Disabled - FAILED" `
            -From $FromAddress `
            -Body "The scheduled licensed+disabled user audit run failed: $($_.Exception.Message)`n`nSee $OutputFile on the host machine for details."
    }
    catch {
