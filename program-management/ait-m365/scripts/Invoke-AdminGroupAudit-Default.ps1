...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\admingroup-audit.csv"
...
try {
    ...
    & (Join-Path $PSScriptRoot "..\01-collector\Collect-EntraAdminGroupMembers.ps1") -Directory $CustomerDir -SettingsPath $SettingsPath
    Write-ToLog -LogFile $OutputFile -Message "Collected Entra role assignments/users/licenses"

    & (Join-Path $PSScriptRoot "..\02-analyst\Compare-AdminGroupAudit.ps1") -Directory $CustomerDir -IncludeDisabledUsers
