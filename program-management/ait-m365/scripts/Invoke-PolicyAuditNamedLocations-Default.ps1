...
.EXAMPLE
    .\Invoke-PolicyAuditNamedLocations-Default.ps1
#>
...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\named-locations-audit.csv"
...
try {
    ...
    Write-ToLog -LogFile $OutputFile -Message "=== Starting $($SpFolder) named locations audit run ==="

    & (Join-Path $PSScriptRoot "..\01-collector\Collect-EntraNamedLocations.ps1") -Directory $CustomerDir -SettingsPath $SettingsPath
    Write-ToLog -LogFile $OutputFile -Message "Collected Entra named locations"

    & (Join-Path $PSScriptRoot "..\02-analyst\Compare-NamedLocationsAudit.ps1") -Directory $CustomerDir
    Write-ToLog -LogFile $OutputFile -Message "Generated the named locations audit"

    # ---------------------------------------------------------------------------
    # Send email
    # ---------------------------------------------------------------------------

    & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra Named Location Audit" `
    ...
    try {
        & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra Named Location Audit - FAILED" `
            -From $FromAddress `
            -Body "The scheduled named locations audit run failed: $($_.Exception.Message)`n`nSee $OutputFile on the host machine for details."
    }
    catch {
