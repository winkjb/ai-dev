...
.EXAMPLE
    .\Invoke-SiteAuditTeams-Activity-Default.ps1
#>
...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\Teams-Activity.csv"
...
try {
    ...
    Write-ToLog -LogFile $OutputFile -Message "=== Starting $($SpFolder) Teams activity audit run ==="
    ...
    & (Join-Path $PSScriptRoot "..\02-analyst\Compare-Teams-Activity.ps1") -Directory $CustomerDir
    Write-ToLog -LogFile $OutputFile -Message "Generated the Teams activity audit"
    ...
    & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Teams Site Activity Audit" `
    ...
    try {
        & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Teams Site Activity Audit - FAILED" `
            -From $FromAddress `
            -Body "The scheduled Teams activity audit run failed: $($_.Exception.Message)`n`nSee $OutputFile on the host machine for details."
    }
    catch {
