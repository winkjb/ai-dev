...
.EXAMPLE
    .\Invoke-PolicyAuditConditionalAccess-Default.ps1
#>
...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\conditional-access-policy-audit.csv"
...
try {
    ...
    & (Join-Path $PSScriptRoot "..\01-collector\Collect-EntraConditionalAccessPolicies.ps1") -Directory $CustomerDir -SettingsPath $SettingsPath
    Write-ToLog -LogFile $OutputFile -Message "Collected Entra Conditional Access policies"

    & (Join-Path $PSScriptRoot "..\02-analyst\Compare-ConditionalAccessPolicyAudit.ps1") -Directory $CustomerDir
