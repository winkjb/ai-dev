...
.EXAMPLE
    .\Invoke-UserAccessAudit-Disable-Default.ps1
#>
...
$AuditCsv = Join-Path $PSScriptRoot "..\02-analyst\output\$($CustomerDir)\UserAudit-AccessDisable.csv"
...
try {
    ...
    Write-ToLog -LogFile $OutputFile -Message "=== Starting $($SpFolder) user access disable audit run ==="
    ...
    # on a large tenant, checking everyone instead of just the stale-login candidates was
    # measured costing minutes instead of seconds. See Compare-Users-Disable.ps1's -ListCandidateUpns.
    $CandidateUpns = & (Join-Path $PSScriptRoot "..\02-analyst\Compare-Users-Disable.ps1") -Directory $CustomerDir -ListCandidateUpns
    Write-ToLog -LogFile $OutputFile -Message "Identified $(@($CandidateUpns).Count) stale-login candidate(s)"

    $ScopedPurposePath = Join-Path $PSScriptRoot "..\data\raw\$($CustomerDir)\EntraMailboxPurpose-AccessDisable.csv"
    ...
    & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra User Access Audit" `
    ...
    try {
        & $EmailScript -To $ToAddresses -Subject "$($SpFolder) - Entra User Access Audit - FAILED" `
            -From $FromAddress `
            -Body "The scheduled user access disable audit run failed: $($_.Exception.Message)`n`nSee $OutputFile on the host machine for details."
    }
    catch {
