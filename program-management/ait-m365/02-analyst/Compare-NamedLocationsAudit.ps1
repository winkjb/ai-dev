...
.SYNOPSIS
    ...
    (../01-collector/Collect-EntraNamedLocations.ps1's raw snapshot) into a flat report - one
    ...
    wrapper's job, respectively (see ../katz/Invoke-PolicyAuditNamedLocations.ps1).
...
.EXAMPLE
    .\Compare-NamedLocationsAudit.ps1 -Directory katz
#>
...
if (-not $OutputPath) { $OutputPath = Join-Path $PSScriptRoot "output\$Directory\named-locations-audit.csv" }
