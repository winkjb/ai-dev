...
.SYNOPSIS
    ...
    respectively (see ../katz/Invoke-MailboxAuditSize.ps1).

.DESCRIPTION
    Soft-deleted mailboxes (IsDeleted=True in the raw snapshot) are always dropped - matches the
    original katz/Invoke-MailboxAuditSize-Default.ps1 script's unconditional behavior (no
    ...
    as ../02-analyst/Compare-AdminGroupAudit.ps1's -LessThanDays default) - override per-call for
...
.EXAMPLE
    .\Compare-MailboxSizeAudit.ps1 -Directory katz
#>
...
if (-not $LicenseCatalogPath)     { $LicenseCatalogPath     = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraLicenseCatalog.csv" }
if (-not $LicenseAssignmentsPath) { $LicenseAssignmentsPath = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraLicenseAssignments.csv" }
if (-not $OutputPath)             { $OutputPath             = Join-Path $PSScriptRoot "output\$Directory\mailbox-size-audit.csv" }
