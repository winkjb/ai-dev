...
.SYNOPSIS
    ...
    (../01-collector/Collect-EntraConditionalAccessPolicies.ps1's raw snapshots) into a flat
    ...
    ../katz/Invoke-PolicyAuditConditionalAccess.ps1).

.DESCRIPTION
    A policy counts as a "Microsoft template policy" when it has a TemplateId and its State is
    "disabled" - matches the original katz/Invoke-PolicyAuditConditionalAccess-Default.ps1
    ...
    ../02-analyst/Compare-TempGroupsAudit.ps1's IsExcluded convention) - pass -ExcludeMsPolicies
    to drop them entirely instead, matching the original script's switch of the same name.

.EXAMPLE
    .\Compare-ConditionalAccessPolicyAudit.ps1 -Directory katz
#>
...
if (-not $UsersPath)    { $UsersPath    = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraDirectoryUsers.csv" }
if (-not $GroupsPath)   { $GroupsPath   = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraDirectoryGroups.csv" }
if (-not $RolesPath)    { $RolesPath    = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraDirectoryRoles.csv" }
if (-not $PoliciesPath) { $PoliciesPath = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraConditionalAccessPolicies.csv" }
if (-not $OutputPath)   { $OutputPath   = Join-Path $PSScriptRoot "output\$Directory\conditional-access-policy-audit.csv" }
