...
.SYNOPSIS
    ...
    (../01-collector/Collect-EntraCaGroupMembers.ps1's raw snapshot) against the expected
    ...
    ../katz/Invoke-CaGroupMemberAudit.ps1).
...
.DESCRIPTION
    ...
    baseline from the original katz/Invoke-CaGroupMemberAudit-Default.ps1 script; override
...
.EXAMPLE
    .\Compare-CaGroupAudit.ps1 -Directory katz
#>
...
param(
    ...
    # Baseline as of the original katz/Invoke-CaGroupMemberAudit-Default.ps1 script (retired
...
if (-not $RawPath)      { $RawPath      = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraCaGroupMembers.csv" }
if (-not $ReferenceDir) { $ReferenceDir = Join-Path $PSScriptRoot "..\data\reference\$Directory" }
if (-not $OutputPath)   { $OutputPath   = Join-Path $PSScriptRoot "output\$Directory\cagroup-audit.csv" }
