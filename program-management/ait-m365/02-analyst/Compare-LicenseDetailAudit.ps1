...
.SYNOPSIS
    ...
    ../katz/Invoke-LicenseAuditDetail.ps1).
...
.DESCRIPTION
    ...
    -IncludeExclusions is used, matching ../02-analyst/Compare-TempGroupsAudit.ps1's convention).

.EXAMPLE
    .\Compare-LicenseDetailAudit.ps1 -Directory katz
#>
...
if (-not $CatalogPath)     { $CatalogPath     = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraLicenseCatalog.csv" }
if (-not $AssignmentsPath) { $AssignmentsPath = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraLicenseAssignments.csv" }
if (-not $ExclusionsPath)  { $ExclusionsPath  = Join-Path $PSScriptRoot "..\data\reference\$Directory\excluded-licenses.csv" }
if (-not $OutputPath)      { $OutputPath      = Join-Path $PSScriptRoot "output\$Directory\license-detail-audit.csv" }
