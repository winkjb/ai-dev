...
.SYNOPSIS
    ...
    (../01-collector/Collect-EntraAdminGroupMembers.ps1's raw snapshots) against the internal
...
.EXAMPLE
    .\Compare-AdminGroupAudit.ps1 -Directory katz
#>
...
if (-not $RoleAssignmentsPath) { $RoleAssignmentsPath = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraRoleAssignments.csv" }
if (-not $UsersPath)           { $UsersPath           = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraUsers.csv" }
if (-not $LicensesPath)        { $LicensesPath         = Join-Path $PSScriptRoot "..\data\raw\$Directory\EntraLicenses.csv" }
if (-not $OutputPath)          { $OutputPath           = Join-Path $PSScriptRoot "output\$Directory\admingroup-audit.csv" }
