################################################################################################################### 
##
## This script audits ServIT admin users in Vipre Email Security
## Version 1.2
##
...
$LogFile = "C:\PS\servit-msp\Logs\VipreAdminAudit-ServIt.csv" # ie. c:\mylog.csv
$ToEmailAddr = @("bwinklesky@servit.net","rpardue@servit.net","jmatthews@servit.net") # Multiple addr allowed but MUST be independent strings separated by comma
##
################################################################################################################### 
...
$Uri = "https://portal.mailanyone.net/rest/public/customer/"+$CustomerSettings.VipreAccountId+"/admin?num=1000&offset=0"
...
foreach ($Admin in $Admins) {
    ...
    Write-Progress -activity "Processing..." -status "$i out of $CountAdmins completed"
...
if ($Logging -eq $true) {
    ...
    $Results | Select Date,CustomerName,FirstName,LastName,Username,Role,LastLogin,DaysAgo,Enabled | Sort Role,LastLogin,FirstName | Export-CSV -Path $LogFile -NoTypeInformation
    ...
    $To          = $ToEmailAddr
    $Subject     = "Vipre Admin Audit - ServIT"
    $Body        = $Body
